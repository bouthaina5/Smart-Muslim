 ...
 import {hideCandidateWords, plotPredictions, plotSpectrogram, populateCandidateWords, showCandidateWords} from './ui';
...
const startTransferLearnButton = async () => {
...
const evalModelOnDatasetButton = async () => {
    const files = datasetFileInput.files;
    if (files == null || files.length !== 1) {
      throw new Error('Must select exactly one file.');
    }
...
const saveTransferModelButton =  async () => {
    await transferRecognizer.save();
    await populateSavedTransferModelsSelect();
    saveTransferModelButton.textContent = 'Model saved!';
    saveTransferModelButton.disabled = true;
...
const loadTransferModelButton = async () => {
    const transferModelName = savedTransferModelsSelect.value;
    await recognizer.ensureModelLoaded();
    transferRecognizer = recognizer.createTransfer(transferModelName);
    await transferRecognizer.load();
...
//  });
 ...
 const deleteTransferModelButton = async () => {
    const transferModelName = savedTransferModelsSelect.value;
    await recognizer.ensureModelLoaded();
    transferRecognizer = recognizer.createTransfer(transferModelName);
    await SpeechCommands.deleteSavedTransferModel(transferModelName);
...
<div>
    <div className="start-stop" >
        <button id="start" disabled={true} onClick={startButton}>Start</button>
        <button id="stop" disabled={true}onClick={stopButton}>Stop</button>
    </div>
    <div className='main-model'>
        <div className="settings">
            <span >Prob. threshold:</span>
            <input className="settings" size="5" id="proba-threshold" value="0.75"/>
        </div>
        <div id="candidate-words" className="candidate-words-hidden"></div>
    </div>
    <div className="footer">
        <textarea id="status-display" cols="80" readOnly={false} ></textarea>
    </div>

    <div className="transfer-learn-section">
        <input id="transfer-model-name" size="20" placeholder="model name" onChange={()=>{}}/>
        <input id="learn-words" size="36" value="_background_noise_,red,green" onChange={()=>{console.log('input words')}}/>
        <select id="duration-multiplier" defaultValue={'2'}>
        ...
        <div className="collapsible-region">
            <button id="dataset-io" onClick={datasetIOButton}>Dataset IO </button>
            <div className="collapsible-region-inner" id="dataset-io-inner">
            <div>
                <button id="download-dataset" disabled={true} onClick={downloadAsFileButton}>↓ Download dataset as file</button>
                <div>
                    <input type="file" id="dataset-file-input"/>
                    <button id="upload-dataset" onClick={uploadFilesButton}>↑ Upload dataset</button>
                    <button id="eval-model-on-dataset" onClick={evalModelOnDatasetButton}>Evaluate model on dataset</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div className="settings">
      <span>Epochs:</span>
      <input className="settings" size="5" id="epochs" value="100"/>
      <span>Fine-tuning (FT) epochs:</span>
      <input className="settings" size="5" id="fine-tuning-epochs" value="0"/>
      <span>Augment by mixing noise:</span>
      <input type="checkbox" id="augment-by-mixing-noise"/>
      <button id="start-transfer-learn" disabled={true} onClick={startTransferLearnButton}>Start transfer learning</button>
    </div>

    <div id="plots">
      <div id="loss-plot" className="plots"></div>
      <div id="accuracy-plot" className="plots"></div>
      <div>
        <div>
          <span id="eval-results" className="eval-results"></span>
        </div>
        <div id="roc-plot" className="plots"></div>
      </div>
    </div>

    <div className="collapsible-region">
      <button id="model-io">Model IO</button>
      <div className="collapsible-region-inner" id="transfer-model-save-load-inner">
        <div>
          <button id="load-transfer-model" disabled={true} onClick={loadTransferModelButton}>Load:</button>
          <select id="saved-transfer-models">
            <option value="1"></option>
          </select>
          <button id="delete-transfer-model" disabled={true} onClick={deleteTransferModelButton}>Delete</button>
        </div>
        <div>
          <button id="save-transfer-model" disabled={true} onClick={saveTransferModelButton}> Save model</button>
        </div>
      </div>
    </div>
  </div>)
